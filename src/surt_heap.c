...
void sift_top_down( Heap *h, int32_t idx )
{
    ...
    if ( h->flag == true ) {
        ...
        while ( true ) {
            ...
            r = RCHILD_IDX( i );
            if ( ( l < h->size ) &&
                 ( h->fn_cmp( arr + h->elem_sz * l, arr + h->elem_sz * min ) <
                   0 ) ) {
                min = l;
            }
            if ( ( r < h->size ) &&
                 ( h->fn_cmp( arr + h->elem_sz * r, arr + h->elem_sz * min ) <
                   0 ) ) {
                min = r;
            }
            if ( min == i ) {
        ...
        while ( true ) {
            ...
            r = RCHILD_IDX( i );
            if ( ( l < h->size ) &&
                 ( h->fn_cmp( arr + h->elem_sz * max, arr + h->elem_sz * l ) <
                   0 ) ) {
                max = l;
            }
            if ( ( r < h->size ) &&
                 ( h->fn_cmp( arr + h->elem_sz * max, arr + h->elem_sz * r ) <
                   0 ) ) {
                max = r;
            }
            if ( max == i ) {
 ...
 */
int32_t surt_heap( void    *arr_ptr,
                   uint32_t arr_len,
                   uint32_t elem_sz,
                   cmp_fn_p fn_cmp,
                   trv_fn_p fn_trv,
