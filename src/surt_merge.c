...
#define SET_SURT_INFO(_info, _obj, _sz, _len, _begin, _mid, _end, _opt, _fcmp, \
                      _ftrv, _buf)                                             \
  do {                                                                         \
    (_info)->surt_obj = (_obj);                                                \
    (_info)->obj_sz = (_sz);                                                   \
...
static void surt_merge_conquer(MergeInfo *info) {
  ...
  memcpy(info->tmp_buff, info->surt_obj + idx1 * info->obj_sz,
  ...
  if ((info->option & SOPT_ORDER_MASK) == SOPT_INVERSE_ORDER) {
    ...
    while (idx1 <= info->abs_middle && idx2 <= info->abs_end) {
      if (info->fn_cmp(info->tmp_buff + (idx1 - offset) * info->obj_sz,
                       info->surt_obj + idx2 * info->obj_sz) <= 0) {
        memcpy(ans + idx3 * info->obj_sz,
               info->tmp_buff + (idx1 - offset) * info->obj_sz, info->obj_sz);
        idx3++;
        idx1++;
      } else {
        memcpy(ans + idx3 * info->obj_sz, info->surt_obj + idx2 * info->obj_sz,
               info->obj_sz);
        idx3++;
        idx2++;
      }
...
static void surt_merge_divide(MergeInfo *info) {
  uint32_t low = info->abs_begin, mid = info->abs_middle, high = info->abs_end;
  MergeInfo tmp_info;
  ...
  if (low < high) {
    SET_SURT_INFO(&tmp_info, info->surt_obj, info->obj_sz, info->obj_len, low,
                  low + (mid - low) / 2, mid, info->option, info->fn_cmp,
                  info->fn_trv, info->tmp_buff);
    surt_merge_divide(&tmp_info);

    SET_SURT_INFO(&tmp_info, info->surt_obj, info->obj_sz, info->obj_len,
                  mid + 1, mid + 1 + (high - mid - 1) / 2, high, info->option,
                  info->fn_cmp, info->fn_trv, info->tmp_buff);
    surt_merge_divide(&tmp_info);
...
#ifdef DEBUG_MODE
    if (info->fn_trv != NULL) {
      msg_len = snprintf(_merge_sort_msg_buff, 128, "Conquer %3d",
                         _merge_sort_conquer_cnt);
...
int32_t surt_merge_r(void *arr_ptr, uint32_t arr_len, uint32_t elem_sz,
  ...
  if (arr_len > 1) {
    ...
    SET_SURT_INFO(&info, arr_ptr, elem_sz, arr_len, low, mid, high, option,
                  fn_cmp, fn_trv, tmp_elems);
